@extends('layouts.admin', ['title' => $stage->title])
...
@section('content')
    ...
    <form action="{{ route('admin.crm.stages.update', [$project, $stage]) }}" method="post">
        @csrf
        @method('patch')
        ...
        <div class="form-group">
            ...
            <input type="text" name="title"
                   ...
                   value="{{ old('title') ?? $stage->title }}" required>
        </div>

        <div class="form-group">
            <label for="description">Описание</label>
            <wysiwyg name="description" content="{{ old('description') ?? $stage->description }}"></wysiwyg>
        </div>
            ...
            <div class="col-sm">
                <div class="form-group">
                    ...
                    <input type="datetime-local" name="starts_at" class="form-control"
                           value="{{ old('starts_at') ?? optional($stage->starts_at)->toDatetimelocalString() }}">
                </div>
            </div>
            ...
            <div class="col-sm">
                <div class="form-group">
                    ...
                    <input type="datetime-local" name="ends_at" class="form-control"
                           value="{{ old('ends_at') ?? optional($stage->ends_at)->toDatetimelocalString() }}">
                </div>
            </div>
        </div>
                ...
                <select name="est_currency" id="est_currency" class="form-control w-auto mr-1">
                    @foreach(\App\Models\Base::$currencies as $currency)
                        <option value="{{ $currency }}"
                            {{ $currency === $stage->est_currency ? 'selected' : '' }}>
                            {!! __('crm.currencies.'.$currency) !!}
                        </option>
                    @endforeach
                </select>

                <input type="number" name="est_price" class="form-control w-auto"
                       value="{{ old('est_price') ?? $stage->est_price }}">
            </div>
        </div>
